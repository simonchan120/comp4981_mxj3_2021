...
import { NavigationActions, createNavigator, StackRouter, SwitchView } from '@react-navigation/core';
...
describe('NavigationContainer', () => {
  ...
  describe('state persistence', () => {
    async function createPersistenceEnabledContainer(loadNavigationState, persistNavigationState = jest.fn()) {
      const navContainer = renderer.create(<NavigationContainer persistNavigationState={persistNavigationState} loadNavigationState={loadNavigationState} />).getInstance();
    ...
    it('loadNavigationState is called upon mount and persistNavigationState is called on a nav state change', async () => {
      ...
      const navigationContainer = await createPersistenceEnabledContainer(loadNavigationState, persistNavigationState);
    ...
    it('when persistNavigationState rejects, a console warning is shown', async () => {
      ...
      const persistNavigationState = jest.fn().mockRejectedValue(new Error('serialization failed'));
    ...
    it('when loadNavigationState rejects, navigator ignores the rejection and starts from the initial state', async () => {
      const loadNavigationState = jest.fn().mockRejectedValue(new Error('deserialization failed'));

      const navigationContainer = await createPersistenceEnabledContainer(loadNavigationState);
    ...
    it('throws when persistNavigationState and loadNavigationState do not pass validation', () => {
      expect(() => renderer.create(<NavigationContainer persistNavigationState={jest.fn()} />)).toThrow();
    });
  });
});